...
// Эндпоинты, где 401 НЕ должен инициировать refresh: по спеке у них нет схемы безопасности
// (security: []), поэтому 401 они не отдают вовсе. Категорий тут две, и продление не лечит ни ту,
// ни другую:
//  - guests-only — метод предназначен НЕавторизованному (вход, регистрация, открытие сессии);
//    авторизованной вкладке он отказывает по 403 «вы уже авторизованы»;
//  - расширения нет — метод доступен и гостю, и авторизованному, право на действие даёт секрет
//    в самом запросе (методы операции — сам токен операции). Отказ приходит по 400 в теле,
//    403 у них нет вовсе.
// Граница между категориями — именно допустимость авторизованного вызывающего, а не источник
// права: открытие сессии тоже идёт по токену операции, но авторизованному запрещено.
...
// Группа /v1/check/* целиком без схемы безопасности, поэтому матчится префиксом.
 ...
 *
 * Обработки ошибок у этих параметров нет и быть не может: по спеке общие параметры запроса не
 * могут стать причиной отказа — неподдерживаемое значение сервер молча игнорирует и берёт
 * следующий источник по приоритету. `ValidateError/lang` и `ValidateError/tz` бывают только по
 * одноимённым полям ТЕЛА `POST /v1/user/settings`: там значение сохраняется в профиль, а не
 * применяется к одному ответу.
 *
