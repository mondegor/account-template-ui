...
// Эндпоинты, где 401 НЕ должен инициировать refresh: по спеке они гостевые (security: []) и 401
// не отдают вовсе. Отказать они могут только по 403 «вы уже авторизованы» (вход, регистрация,
// открытие сессии) либо по 400 в теле (методы операции, где право даёт сам токен операции, —
// у них 403 нет вовсе); продление не лечит ни то, ни другое.
