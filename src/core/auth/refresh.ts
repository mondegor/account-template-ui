 ...
 * проактивный по таймеру) сливаются в один PATCH /v1/session. Успех — любой 2xx, источник истины
 * тело ответа (SuccessAccessBody): спека объявляет 201, моки отдают 200, и завязываться на
 * конкретный код нам незачем. Серверное grace-окно (60с, повтор тем же токеном возвращает те же
 * токены) делает повторы идемпотентными, поэтому navigator.locks не нужен.
 */
