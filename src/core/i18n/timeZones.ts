...
/**
 * Справочник часовых поясов — **копия серверного списка** (он собран из списка часовых поясов
 * Windows). Совпадение обязательно: явно выбранное значение `tz` бэк проверяет строго по своему
 * списку — чужое имя вернулось бы 400 на сохранении настроек. В query-параметре неизвестное имя
 * не ошибка: оно пропускается, и пояс берётся из следующего по приоритету источника. Меняется
 * список на бэке — пересобирается и этот файл.
 *
