 ...
 * через перезагрузку. Код (secret) НЕ храним. Абсолютные таймеры позволяют пересчитать остаток
 * от now. sessionStorage (не cookie): на сервер слать не нужно, JS должен читать; чистится при
 * закрытии вкладки.
 */
