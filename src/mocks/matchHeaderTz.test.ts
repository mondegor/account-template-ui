 ...
 * неизвестно. Мок повторяет алгоритм бэка (go-core/util/timezone.NameByOffset): индекс строится
 * по СОСТОЯНИЯМ зон (смещение + летнее время действует сейчас), поэтому у зоны с переходом
 * в индексе две записи, а при коллизии побеждает зона, встреченная в списке позже.
 *
