...
/**
 * Индекс подбора зоны по паре (смещение, летнее время действует) — как
 * `go-core/util/timezone.LocationList.NameByOffset`. Ключ — СОСТОЯНИЕ зоны, а не её описание:
 * у зоны с переходом состояний два, и оба лежат в индексе. Поэтому летний Стамбул
 * (`+03:00`, dst=0) не путается с Каиром: у того в июле состояние (`+03:00`, dst=1),
 * а в январе (`+02:00`, dst=0).
 *
