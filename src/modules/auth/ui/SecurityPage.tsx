 ...
 *
 * Наружный заголовок называет РАЗДЕЛ: установка пароля, перевыпуск аварийных кодов и отключение —
 * всё это двухфакторная защита, она и стоит по умолчанию. Смена емаила и телефона к ней не
 * относится и начинается в настройках — такие потоки называют раздел сами. Поток называет шапка
 * карточки, и глиф метит там его же — щит с точками у пароля, спаскруг у аварийных кодов. Тон у
 * глифа брендовый, как у знака секции в профиле: на цвете текста он сливался бы с заголовком рядом.
 *
...
export function SecurityPage({
  ...
  /** Заголовок раздела над карточкой; без него — двухфакторная защита. */
  section?: string;
  icon?: ReactNode;
  title?: string;
  /**
