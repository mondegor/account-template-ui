        ...
        {on ? (
          <Stack spacing={1.5} sx={{ mt: 1.5, alignItems: 'flex-start' }}>
            ...
            <Button
              ...
              onClick={() => flow.mutate({ kind: 'disable2fa', start: startDisable2fa })}
...
/**
 * Предложение метода: щит и название сверху, «как работает», плюс и минус, призыв по низу справа.
 * Доступный метод зовёт дальше — на свой экран либо сразу на подтверждение, если спрашивать до
 * операции нечего. Включённому, как и затенённому, предложить нечего: защиту снимает кнопка внизу
 * карточки.
 *
 * Сама плитка — описание метода, а не кнопка: нажимается только ссылка внизу. Поэтому и цвет у неё
 * не тон метода: тоном плитка размечена (щит и полоска слева), а ссылка идёт брендовым.
 */
function MethodTile({
  method,
  mode,
  ...
  onStart?: () => void;
  /**
   * Действие над защитой уже идёт — второе начинать нечем. Знака занятости у призыва нет, даже когда
   * он зовёт сервер: спиннер посреди строки текста читается мусором, а рост знака занятости задан
   * по кнопке и рядом с шевроном дёргал бы подпись. Ответ на нажатие — погасшая ссылка, а следом
   * экран, который открывает поток.
   *
  ...
  // Шеврон значит переход: призыв ведёт всегда — на свой экран метода или сразу на экран
  // подтверждения.
  const callSx = {
    display: 'inline-flex',
    alignItems: 'center',
